...
近年ではよりミニマルでFEフレンドリーなHeadless CMSなどが登場してはいるものの、プロダクトで利用する場合はやはり圧倒的シェアを誇るWordPressの豊富な情報やプラグインの存在から「だいたいの要件がWPで実現できる」という信頼と実績は捨てがたい。

たとえば、コーポレートサイトにおける「投稿可能な『実績紹介ページ』」や「お問い合わせフォーム」はWPがあれば難なく作れるだろう。
...
私はキャリア2年半程度のHTMLコーダー上がりのへっぽこFEだが、WordPressには少なからず苦手意識がある。というのも、
...
怒涛の勢いで出てくる見慣れない文字列は、WordPressの組み込み関数である。WordPressのこういうところはちょっと「ウッ」となる部分でもあるのだが、[リファレンス](https://wpdocs.osdn.jp/%E9%96%A2%E6%95%B0%E3%83%AA%E3%83%95%E3%82%A1%E3%83%AC%E3%83%B3%E3%82%B9)は充実しているのでにらめっこしながらがんばって使っていこう。`wp_enqueue_style`の第一引数は一意な識別名のようなもので、被らなければよい。
...
であり、「実績紹介」と「ニュース」という**2種類**の投稿画面が必要になる。データの持ち方についても、たとえばコンテンツごとに「執筆者」といった追加の項目が必要な場合もあるだろう。

結論として、管理したいコンテンツが2種類以上ある、さらにコンテンツごとに持ちたい項目が変わる場合は「カスタム投稿タイプ」や「カスタムフィールド」の作成が必要となる。
...
今回はブログのように「投稿型コンテンツがサイトの中核となる」わけではなく、「通常のWebサイトにいくつか投稿型のコンテンツがくっついている」ようなページを作成する。そのためデフォルトの「投稿」はいったん利用せずに、「実績紹介」と「ニュース」をカスタム投稿タイプで管理していくことにする（詳細な解説はあとに回すが、そのほうがパーマリンクの挙動や作成するファイルの命名が直感的となるためだ）。

さっそくカスタム投稿タイプの追加を行いたいのだが、これが
